        ...
        for i=0, objects:size() - 1 do
            ...
            if container and not container:isEmpty() then
                ...
                for i=0, items:size()-1 do
                    ...
                    if BWOUtils.IsUsefulItem(item) then
                        zinventory:AddItem(item)
                    else
                        local bsquare = zombie:getSquare()
                        bsquare:AddWorldInventoryItem(item, ZombRandFloat(0.1, 0.9), ZombRandFloat(0.1, 0.9), 0)
                    end
                end

                BWOInteractables.Remove(task.x, task.y, task.z)
